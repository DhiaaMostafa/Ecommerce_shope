...
$("#home_searchResult option").each(function(){//duplicated not allowed
  ...
  s=0;
            ...
            if(s)
				...
				$("#home_searchResult option").bind("click",function(){
...
$("#search_button").click(function(){//applay filter search
  var search=$("#txt_search").val();

  $(".product_image").each(function(){
    if($(this).find(".imginfo").text()!==search)
    {$(this).remove();
   
 } });});

$("#txt_search").keyup(function(e){//show a filter search suggest when keyup
...
$(".imginfo").each(function(){
  ...
  var disc=$(this).text();
        var s=1;
$("#searchResult option").each(function(){
  if($(this).val()==disc)
  s=0;
            });
            if(s)
					$("#searchResult").append("<option value='"+disc+"'>"+"</option>");

			
  
  
          });
          $("#searchResult option").bind('click',function(){
...
{
    $("#id01").load("HTML/pug/logout.html", function(responseTxt, statusTxt, xhr){
      if(statusTxt == "success")
         {   $("#id01").show();
          $("#id01").on('click', function(e) {
